...
> תוכנית העבודה העסקית/פיילוט/סקייל מרוכזת ב־[PILOT_TO_SCALE_WORK_PLAN.md](./PILOT_TO_SCALE_WORK_PLAN.md). `PROJECT_STATE.md` נשאר מקור אמת למצב הנדסי.

### מה כבר סגור

- **`/terms` ו־`/privacy`** — באוויר בפרודקשן; קישורים ב־login, join landing ופוטר דשבורד `/app`
- **Pilot Control Room** — `/admin/pilot` פעיל בפרודקשן
- **מדדי פיילוט** — מצרפיים בלבד, ללא PII, ללא שמות, אימיילים, סכומים, `uid`, `cardId`, `entryId`, טוקנים או מסמכים מלאים
- **Snapshot ידני למדדים** — שמירת תמונת מצב מצרפית בלבד והשוואת שינוי מול snapshot קודם
- **כרטיס “שמור למסך הבית” בדשבורד** — וריאנט רחב של `InstallAppButton`, ללא קומפוננטה כפולה; מוסתר אם האפליקציה מותקנת / standalone
- **יצירת כרטיס** — תמיד שואלת איך היוצר רוצה להופיע לצד השני; לא משתמשת באימייל כשם ולא ממציאה שם לבד
- **מיון דשבורד** — כרטיסים שממתינים לאישור המשתמש מקבלים קדימות
- **כרטיס “פתח כרטיס עם מישהו אחר”** — מוצג למשתמש שהוזמן ועדיין לא יצר כרטיס משלו
- **WhatsApp Preview / קישור הזמנה** — פעילים; `/j/{token}` נתיב ראשי, `/join/{token}` legacy נשמר
- **הודעת הזמנה קצרה (Mode A)** — נוסח מאושר:
  > פתחתי לנו כרטיס חשבון. כנס כאן 👇🏼
- **דשבורד וכרטיסים** — עובדים אחרי תיקון indexes הנדרשים
...
- **[PILOT_TO_SCALE_WORK_PLAN.md](./PILOT_TO_SCALE_WORK_PLAN.md)** — תוכנית המשך מפיילוט 3 משתמשים עד סקייל משמעותי

### מה פתוח לפני פיילוט אמיתי

| נושא | תיאור |
|------|--------|
| **בדיקת flow מלאה אחת מקצה לקצה** | יצירת כרטיס → שליחת הזמנה → הצטרפות → יצירת פעולה → אישור פעולה → בדיקת Control Room → בדיקת snapshot → בדיקת שמירה למסך הבית |
| **בחירת 3 משתמשים ראשונים** | להפריד בין התחשבנות אמיתית לבין בעל עסק / נבדק כללי |
| **שליחה לאדם ראשון בלבד** | לא לשלוח לשלושה יחד לפני בדיקת החיכוך הראשון |
| **מעקב 24–48 שעות** | ללמוד מהאדם הראשון לפני הרחבה לעוד 2 |
...
| מסמך | תפקיד |
|------|--------|
| [PILOT_TO_SCALE_WORK_PLAN.md](./PILOT_TO_SCALE_WORK_PLAN.md) | תוכנית המשך עסקית/פיילוט/סקייל עד 30,000 כרטיסים/חשבונות |
...
1. **א.** בדיקת flow מלאה אחת מקצה לקצה בפרודקשן
2. **ב.** בחירת 3 משתמשים ראשונים וסיווג Mode A/B
3. **ג.** שליחה לאדם ראשון בלבד
4. **ד.** מעקב 24–48 שעות + Control Room + snapshot
5. **ה.** הרחבה לעוד 2 רק אחרי שאין חור חוסם

### כלל עבודה

כל פיתוח חדש לפיילוט חייב **להתיישר מול סעיף זה** ב־`docs/PROJECT_STATE.md` ועם [PILOT_TO_SCALE_WORK_PLAN.md](./PILOT_TO_SCALE_WORK_PLAN.md).  
**לא מוסיפים עוד פיצ׳רים לפני 3 משתמשים**, אלא אם מתגלה חור חוסם.  
לא ליצור מסמכי Source of Truth כפולים: תכנון עומק עסקי/סקייל נשאר במסמך ההמשך; מצב הנדסי נשאר כאן.
