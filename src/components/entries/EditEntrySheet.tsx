...
import { editEntry } from "@/lib/entries/editEntry";
import type { EntryMutationCallableResult } from "@/lib/firebase/functions";
import { entriesCopy } from "@/lib/entries/entriesCopy";
import { getCreatorIntent } from "@/lib/entries/entryIntent";
import type { AccountCardEntryWithId, EntryIntent } from "@/types/entry";

type EditEntrySheetProps = {
  cardId: string;
  entry: AccountCardEntryWithId | null;
...
export function EditEntrySheet({
  ...
  const [selectedIntent, setSelectedIntent] = useState<EntryIntent | null>(() =>
    entry ? getCreatorIntent(entry, balancePerspectiveUid) : null
  );
      ...
      try {
        ...
        setError(
          err instanceof Error
            ? err.message
            : "לא הצלחנו לשמור את השינוי. נסה שוב."
        );
      } finally {
        setPending(false);
      }
    },
    [entry, selectedIntent, amountRaw, title, user, cardId, onEdited, handleClose]
  );
